...
let AuthService = AuthService_1 = class AuthService {
    constructor(jwtService, passwordStrategy, oauthStrategy, sessionRepository, sessionLogRepo, authRepo, authIdentifierRepo, otpProvider, otpProviderEmail, otpProviderPhone, mfaRepo, options, notificationProvider, eventEmitter) {
        ...
        this.createSessionLog = (_a = this.options.createSessionLogOnInvalid) !== null && _a !== void 0 ? _a : false;
    }
    ...
    async createSessionLogFromSessionIfEnabled({ event, reason, session, }) {
        if (!this.createSessionLog)
            return;
        const sessionLog = await this.sessionLogRepo.create(Object.assign(Object.assign({}, session), { sessionId: session.id, event,
            reason }));
    ...
    async invalidateSessions({ uid, event, reason, }) {
        ...
        if (this.createSessionLog) {
            const logs = sessions.map((session) => (Object.assign(Object.assign({}, session), { id: undefined, sessionId: session.id, event,
    ...
    async signup({ dto, uid, userAgent, ip, namespace }) {
        ...
        const triggerVerification = isPasswordless ||
            (this.options.verificationRequired && !(identifier === null || identifier === void 0 ? void 0 : identifier.isVerified)) ||
            has2FA;
        const { secretHash } = auth, filteredAuth = __rest(auth, ["secretHash"]);
        if (triggerVerification) {
            if (this.notificationProvider) {
                if (!(identifier === null || identifier === void 0 ? void 0 : identifier.isVerified) || has2FA || isPasswordless) {
        ...
        return Object.assign(Object.assign({}, tokens), { auth: filteredAuth });
    }
    ...
    async sendVerification(auth, currentIdentifier) {
        ...
        if (!this.notificationProvider && !this.otpProvider && !this.otpProviderEmail && !this.otpProviderPhone)
            return;
        let primaryIdentifier = (currentIdentifier === null || currentIdentifier === void 0 ? void 0 : currentIdentifier.type) !== 'USERNAME' ? currentIdentifier : null;
        ...
        if (!primaryIdentifier) {
            ...
            primaryIdentifier = (_a = fullAuth === null || fullAuth === void 0 ? void 0 : fullAuth.identifiers) === null || _a === void 0 ? void 0 : _a.find(id => id.type === 'EMAIL' || id.type === 'PHONE');
        }
        if (!primaryIdentifier) {
    ...
    async refreshTokens({ refreshToken, currentUserAgent, currentIp, namespace }) {
        try {
            ...
            const resolvedNamespace = namespace !== null && namespace !== void 0 ? namespace : payload.namespace;
            ...
            await this.sessionRepository.update(session.id, {
                ...
                expiresAt: newExpiry,
                ipAddress: currentIp !== null && currentIp !== void 0 ? currentIp : session.ipAddress,
            });
            if (this.eventEmitter) {
    ...
    async updatePassword(uid, dto, userAgent, ip) {
        ...
        if ((_a = this.notificationProvider) === null || _a === void 0 ? void 0 : _a.sendPasswordChangedNotification) {
    ...
    async requestMagicLink(dto) {
        ...
        if (((_a = this.notificationProvider) === null || _a === void 0 ? void 0 : _a.sendMagicLink) && issueResult.code) {
    ...
    async enrollMfa(uid, type) {
        ...
        if (mfa === null || mfa === void 0 ? void 0 : mfa.isEnabled) {
    ...
    async deactivateMfa(uid, type) {
        ...
        if (this.eventEmitter) {
            this.eventEmitter.emit(auth_events_1.AuthEvents.MFA_DEACTIVATED, { uid, type });
        }
